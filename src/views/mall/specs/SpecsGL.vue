<template>
  <div>
    ...
    <!-- 添加信息 -->
    <div class="add">
      <el-button @click="$router.push('/specs/addspecs')" type="primary"
        >添加</el-button
      >
    </div>
    <el-table :data="userList" style="width: 100%" stripe border row-key="id">
      ...
      <el-table-column prop="attrs" label="商品属性" width="600">
        <template  slot-scope="item">
          <el-tag v-for="(item,i) in item.row.attrs" :key="i">
            {{item}}
          </el-tag>
        </template>
      </el-table-column>
...
<script>
// 网络请求  商品规格列表
import { SpecsList, SpecsDel, SpecsTotal } from "network/specs/specs";

export default {
  async created() {
    // 页面创建前  请求  分页请求 size:每页数量   page页码数（必须参数） 从1开始
    // 默认请求第一页
    await this.getSpecsList(this.currentPage);
    // 获取页面总数据 数量
    let totalNum = await SpecsTotal();
  ...
  methods: {
    ...
    async del(item) {
      ...
      this.$confirm("此操作将永久删除该商品规格, 是否继续?", "提示", {
        ...
        .then(async () => {
          // 页面删除商品规格  重新请求数据库中的数据，更新到页面 (this.SpecsList)
          let res = await SpecsDel(id);
          // 重新获取数据库中的商品规格列表数据，获取到的数据 放到初始数据中-实现页面响应(this.SpecsList = data.list;)
          if (res.code == 200) {
            // 删除以后 渲染当前所处页面数据，（不会发生错误）
            this.getSpecsList(this.currentPage);
            // 弹出框 提示商品规格
            this.$message({
              type: "success",
              message: "删除成功!",
    ...
    async changePage(num) {
      ...
      await this.getSpecsList(num);
    },
    // get 获取商品规格列表
    async getSpecsList(page) {
      // 获取商品规格列表数据  获取树形结构  istree ： 1  el-table 添加 row-key="id" 属性即可
      let res = await SpecsList({ size: this.pageSize, page });
      // 获取到的数据 放到初始数据中
